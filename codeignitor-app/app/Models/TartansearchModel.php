<?php
	...
	class TartansearchModel extends Model {


		function getList() {

			$q1 = $this->db->query("SELECT * FROM `family_names` ORDER BY `family_name` ASC");
		...
		function getTartanList() {
			...
			$q1 = $this->db->query("SELECT * FROM `family_names`  ORDER BY `family_id` ASC");
		...
		function getName($nameId) {
			...
			$q1 = $this->db->query("SELECT * FROM `family_names` WHERE (`family_id` = '$nameId')");
		...
		function getLocationById($locationId) {
			...
			$familyLocation = array();
			$q2 = $this->db->query("SELECT * FROM `family_locations` WHERE (`locations_id` = '$locationId')");

			if($q2->num_rows() > 0) {
				$i = 0;

				foreach($q2->result() as $location) {
		...
		function getTartan($tartanId) {
			...
			$q1 = $this->db->query("SELECT * FROM `tartans` WHERE `tartan_id` = '$tartanId'");
			$name = array();
		...
		function getClanById($clanId) {
			$q1 = $this->db->query("SELECT `family_id`, `family_name`, `family_description`, `family_shortdescription`  FROM `family_names` WHERE `family_id` = '$clanId'");
		...
		function getTartans($parentId) {
			...
			$q1 = $this->db->query("SELECT * FROM `tartans` WHERE `tartan_parent` = '$parentId'");
			$tartan = array();
		...
		function getCloudinaryImages($tartanname, $size) {
			...
			if($size == 'thumb'):
				...
				*/
				...
				$imagepath = "tartan/swatches/$tartanname.png";
				$image = $cloudinary->image($imagepath)
					->resize(Resize::crop()->width(450)->height(450))
					->toUrl();

// $cloudinary->image('tartan/swatches/'.$tartanname.png)->toUrl();
// 	$image =  cloudinary_url(, array("height"=>450, "width"=>450, "crop"=>"crop"));
		...
		function saveTartan($tartanId) {
			$session = session();
			$session->set('tartanId', $tartanId);
		}

		function unloadSavedTartan() {
			$session = session();
			$session->remove('tartanId');
			return 'removed';
		}
