	...
	class OutfitsModel extends Model {
		...
		function getTartans($code, $hireorbuy) {
			...
			if($code != 'all'):
				...
				if($hireorbuy == 'buy'):
					$query = $this->db->query("SELECT * FROM `tartans` WHERE (`tartan_active` = 1 AND `tartan_parent`=3) AND `tartan_id` <> 301  AND ( `tartan_hirebuy` != 'Buy' ) ORDER BY `tartan_parent` ASC,`tartan_varient` ASC");
				else:
					$q1 = "SELECT * 
						   FROM `tartans` 
						   RIGHT JOIN `family_names`
						   ON tartans.tartan_parent = family_names.family_id
						   WHERE (`tartan_active` = 1)  AND ( `tartan_hirebuy` != 'Buy' ) AND `tartan_id` <> 301
		...
		function getProduct($chosenProduct, $hireorbuy) {
			...
			//echo $chosenProduct;
			$q1 = "SELECT * FROM  `outfit_elements` ,  `outfit_products_types` 
				   WHERE  (`outfit_products_types`.`product_type_id` =  `outfit_elements`.`product_type` && active = 1  && `outfit_elements`.`product_hirebuy` != '$hireorbuyfilter') AND  (`product_type_name` =  '$chosenProduct' ) AND `product_id` <> 135 
				   ORDER BY `product_sortno`";
			$query = $this->db->query($q1);
			$pdata = false;
		...
		private function getOutfitElements($outfitCode) {
			...
			//KSMS7SH2HCZNM8BJALH9H
			//SMS7 SH2 HC ZN M8 B JA LH 9 H

			//kilt,sporan,shirt,neckwear,shoe,jacket,hose,hire_or_buy
			...
			$q1 = $this->db->query("SELECT * FROM `outfit_elements` WHERE (`product_ref` = '$jacketcode' AND `product_type` = '22' AND `product_id`<>135)");
			if($q1->getNumRows() == 1) {
				$q1Result = $q1->getRow();
			...
			if(!$error):
				...
				// Price if standart tartain for kilt is choosen
				$purchaseCost = 1449;
				$hireCost = 99;
				...
				if($kiltupgrade == 0):
					if($jacketupgrade == 1 || $jacketupgrade == 0 ):
						$buyPrice = $buyPrice;
						$hirePrice = 110;

						if($jacketElements['swatch'] == 'crail-black-5.png'){
							$hirePrice = 99;
						}
				...
				*/
				...
				if($kiltupgrade == 2): // premium tartan

					if($jacketupgrade == 0 && ($jacketElements['swatch'] == 'charlie-black-5.png' || $jacketElements['swatch'] == 'charlie-black-3.png' )):
						$hirePrice = 130;
						$buyPrice = $buyPrice + 55;
					elseif($jacketupgrade == 0):
						$buyPrice = $buyPrice + 55;
						$hirePrice = 110;
					endif;

					if($jacketupgrade == 1 && ($jacketElements['swatch'] == 'charlie-black-5.png' || $jacketElements['swatch'] == 'charlie-black-3.png' )) {
						$buyPrice = $buyPrice + 100;
						$hirePrice = 130;
					} elseif($jacketupgrade == 1 && $jacketElements['swatch'] == 'crail-black-5.png'){
						$hirePrice = 110;
					} elseif($jacketupgrade == 1) {
						$buyPrice = $buyPrice + 100;
						$hirePrice = 145;
					}
				...
				if($kiltupgrade == 3): // Pride/mccalls Tartan
					$buyPrice = 1449;

					if($jacketupgrade == 0):
						$buyPrice = 1449;
						$hirePrice = 125;
						if($jacketElements['swatch'] == 'charlie-black-5.png' || $jacketElements['swatch'] == 'charlie-black-3.png' ){
							$hirePrice = 140;
						}
					endif;
