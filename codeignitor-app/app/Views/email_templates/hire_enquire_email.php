...
<tr class="header">
<td ><img src="https://www.mccalls.co.uk/apps/assets/img/outfitdesigner/interface/outfitdesigner-logo-big.png" class="tag"></td>
</tr>
...
<tr >
	<td colspan="2">
		<h1><?php 
			if ($personname){ echo "$personname ";}else{echo "(No name submitted)";}
		?> would like to enquire about hiring an outfit<br/><?php 
			if ($outfitcode){ echo "Outfit Code: $outfitcode";}
		?>
		</h1>
		<p><strong>User Details</strong><br/>
		<?php 
			if ($personname){ echo "Name: $personname ";}
			if ($usersEmail){ echo " Email Address: $usersEmail";}
			if ($telno){ echo '<br/>Tel no: '.$telno.'<br/>';}
		?>
		</p>
		...
		<?php 
			if ($waistSize){ echo 'Waist Size: '.$waistSize.'<br/>';}
			if ($seatSize){ echo 'Seat Size: '.$seatSize.'<br/>';}
			if ($height){ echo 'Height: '.$height.'<br/>';}
			if ($kiltlength){ echo 'Kilt Length: '.$kiltlength.'<br/>';}
			if ($collar){ echo 'Collar Size: '.$collar.'<br/>';}
			if ($chest){ echo 'Chest Size: '.$chest.'<br/>';}
			if ($shoe){ echo 'Shoe Size: '.$shoe.'<br/>';}
			
		?>	
		<p><strong>Address Details</strong><br/>
		<?php 
			if ($address1){ echo 'Address 1: '.$address1.'<br/>';}
			if ($address2){ echo 'Address 2: '.$address2.'<br/>';}
			if ($city){ echo 'City: '.$city.'<br/>';}
			if ($postcode){ echo 'Postcode: '.$postcode.'<br/>';}
		
		?>
	 
 
		<?php 
			if ($branchChosen){ echo '<br/><br/>Branch Chosen: '.$branchChosen;}
		...
		<p class=""><a href="<?php echo "https://www.mccalls.co.uk/outfitbuilder/#ref=$outfitcode";?>"><img src="https://www.mccalls.co.uk/apps/assets/img/outfitdesigner/constants/viewoutfitonline.png"/></a></p>
